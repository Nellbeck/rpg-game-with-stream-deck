...
class InventoryController {
    ...
    async getInventory(req, res) {
        try {
            ...
            const inventory = await inventory_model_1.Inventory.findOne({ characterId }).populate('items.itemId');
            if (!inventory) {
    ...
    async addItem(req, res) {
        try {
            ...
            const item = await item_model_1.Item.findById(itemId);
            if (!item) {
                res.status(404).json({ message: 'Item not found' });
                return;
            }
            const inventory = await inventory_model_1.Inventory.findOne({ characterId });
            if (!inventory) {
                res.status(404).json({ message: 'Inventory not found' });
                return;
            }
            ...
            const existingItemIndex = inventory.items.findIndex(item => item.itemId.toString() === itemId);
